...
class BlacklistEmailView(TemplateView):
    ...
    def get_email(self):
        ...
        if not constant_time_compare(
            expected_digest.hexdigest(), self.kwargs["digest"]
        ):
            raise Http404
    ...
    def post(self, request, *args, **kwargs):
        ...
        BlacklistedEmail.objects.create(
            ...
            confirmation_ip=self.request.META["REMOTE_ADDR"],
        )
        message = format_html(
            "We've blacklisted your address and will never email <em>{0}</em> again. Sorry to have bothered you!",
            email,
        )
...
class MessageSendView(FormView):
    ...
    def form_valid(self, form):
        ...
        message.save()
        message.send_sender_confirmation(
            self.request.is_secure(), self.request.get_host()
        )
    ...
    def get(self, request, *args, **kwargs):
        try:
            message = Message.objects.get(
                identifier=kwargs["identifier"], sender_email_token=kwargs["token"]
            )
...
class MessageRecipientMessageUpdate(UpdateView):
    ...
    def get_queryset(self):
        ...
        return message.filter(
            recipient_email_token=self.kwargs["token"], status__in=valid_status
        )
